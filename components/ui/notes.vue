<template>
  ...
  <div class="notes-pane" v-bind:class="{'closed': !open}">
    <b-container>
        ...
        <b-col>
          <div>
            ...
            <b-button class="notes-add"
                      ...
                      v-bind:disabled="tabType == 'new'"><i class="icon-add" aria-label="plus sign"></i> Add</b-button>
          </div>
          <b-button variant="link"
                    class="notes-close"
                    v-on:click="closeNotes"><i class="icon-close" aria-label="close"></i></b-button>
        </b-col>
      </b-row>
      <b-row class="notes-views">
        <b-col v-if="tabType == 'browse'">
          ...
          <div v-if="notes.length == 0">Click <i class="icon-add" aria-label="plus sign"></i><b>Add</b> to create your first note for this course.</div>
          <b-list-group v-if="notes.length > 0">
            <b-list-group-item v-for="(note, ix) in filteredNotes" v-bind:key="'note-' + ix">
              ...
              <b-btn class="note-edit" v-on:click="editNote(note['_id'])">
                <fa :icon="['fas', 'pen']" aria-label="edit" />
              </b-btn>
              <b-btn class="note-delete"
                     v-on:click="deleteNote(note['_id'])"><i class="icon-trash" aria-label="delete"></i></b-btn>
            </b-list-group-item>
          </b-list-group>
        </b-col>
        ...
        <b-col v-if="tabType == 'edit'">
          <b-form v-on:submit.prevent="editSave">
            ...
            <div class="note-btns">
              <b-btn v-on:click="cancelNote">Cancel</b-btn>
              <b-btn variant="success" type="submit">Save Changes</b-btn>
            </div>
          </b-form>
        </b-col>
      </b-row>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      filteredNotes() {
        return this.notes
          .filter(note => {
            return (
              note.note.toLowerCase().includes(this.search.toLowerCase()) ||
    ...
    methods: {
      ...
      editNote(id) {
        axios.get('https://ota-course-framework.firebaseio.com/user_notes/' + id + '.json').then(res => {
          this.noteEdit = {
            ...res.data,
            _id: id
          }
      ...
      editSave() {
        this.$store.dispatch('updateNote', this.noteEdit).then(() => {
          this.cancelNote()
        })
      },
      deleteNote(id) {
